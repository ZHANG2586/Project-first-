...
class S extends Component{
      ...
      onSubmit(e){
             ...
             if(this.state.username.length>0&&this.state.email.length>0&&this.state.code.length==6){
                ...
                axios.post('http://localhost:3016/yanzheng/genggai',post)
                 .then((res)=>{
                     console.log(res);
                     alert('验证成功！');
      ...
      onsubmit1(e){
            ...
            if(this.state.email==''||this.state.email.length<8||this.state.email.indexOf('@qq.com')!=this.state.email.length-7){
                     alert('请输入绑定的qq邮箱号！');
                     return;
            }
           var post1={
            ...
            axios.post('http://localhost:3016/yanzheng',post1)
            .then((res)=>{
               console.log(res);
