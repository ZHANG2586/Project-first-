...
export default class k extends Component{
    ...
    onChange1(e){
        ...
        var post={
         ...
         axios.post('http://localhost:3016/huiyi/biaodan',post,{withCredentials: true})
            .then((res)=>{
                console.log(res);
    ...
    onSubmit(e){
        ...
        if(this.state.id.length>0&&this.state.huiyiname.length>0&&this.state.number.length>0&&this.state.number[0]<'A'&&this.state.faqiren.length>0&&document.getElementById('test1').value!=''&&document.getElementById('test2').value!=''&&document.getElementById('test1').value<document.getElementById('test2').value){
              ...
              for(var i=0;i<this.state.s.length;i++){
                if(((new Date(this.state.s[i].star_time).getTime())<=(new Date(document.getElementById('test2').value).valueOf()))&&((new Date(this.state.s.end_time).getTime())>=(new Date(document.getElementById('test1').value).valueOf()))){
                       alert('时间冲突了（即将返回主页面，请重新预定会议！）');
                       window.location.href='http://localhost:3000/#/home';
                       return;
                }
                else if(((new Date(this.state.s[i].star_time).getTime())>=(new Date(document.getElementById('test1').value).valueOf()))&&((new Date(this.state.s[i].star_time).getTime())<=(new Date(document.getElementById('test2').value).valueOf())))
                    {
                            alert('时间冲突了！（即将返回主页面，请重新预定会议！）');
                            window.location.href='http://localhost:3000/#/home';
                            return;
        ...
        axios.post('http://localhost:3016/huiyi/biaodan/tianjia',post,{withCredentials:true}).then((res)=>{
            alert('请求已发送！(即将返回主页面！)');
            window.location.href='http://localhost:3000/#/home';
        }).catch((err)=>{
            console.log(err);
    ...
    render(){
        return (
          <div className='s' style={{width: '80%',height:'500px',backgroundColor:'blanchedalmond',position:'relative',left:'0',right:'0',margin:'0 20% 0  10%'}}>
                <input className='id' type='text' id='id' onChange={this.onChange} autocomplete="off" placeholder='请输入会议id'></input>
                <input className='huiyiname' type='text' id='huiyiname' onChange={this.onChange} autocomplete="off" placeholder='请输入会议房间名称'></input>             
                <input className='number' type='text' id='number' onChange={this.onChange} autocomplete="off" placeholder='请输入会议室人数上线'></input>
                <input className='star_time' type='text' id='test1' onChange={this.onChange} onFocus={this.onFocus} autocomplete="off"  placeholder='请输入会议起始时间'></input>
                <input className='end_time' type='text' id='test2' onChange={this.onChange}   onFocus={this.onFocus} onBlur={this.onChange1} autocomplete="off" placeholder='请输入会议结束时间'></input>
                <input className='faqiren' type='text'  id='faqiren' onChange={this.onChange}  autocomplete="off" placeholder='请输入会议发起人名称'></input>
