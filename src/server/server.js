...
app.use(cors({
  ...
  origin:'http://192.168.3.13:51467',
  credentials:true
}));
...
app.all('*', function (req, res, next) {
  //响应头指定了该响应的资源是否被允许与给定的origin共享。*表示所有域都可以访问，同时可以将*改为指定的url，表示只有指定的url可以访问到资源. 
    ...
    res.header('Access-Control-Allow-Origin'," http://192.168.3.13:51467");
    ...
    User.find({'username':req.body.username},function(err,doc){
      if(err){
        console.log(err);
         ...
         User.updateOne({'username':req.body.username,'email':req.body.email},{'username':req.body.username,'password':req.body.password,'email':req.body.email},function(err,doc){
           if(err){
             console.log(err);
           }else{
